...
exports.createAlert = async (req, res) => {
  try {
    const userAlerts = await Alert.countDocuments({
      user: req.user.id,
      isActive: true,
    });
    if (userAlerts >= 10)
      return res
        .status(400)
        .json({ success: false, message: "Maximum 10 alerts allowed" });
    ...
    res
      ...
      .json({
        success: false,
        message: error.message || "Error creating alert",
      });
  }
};
...
exports.getMatchingProperties = async (req, res) => {
  try {
    ...
    if (alert.criteria.listingType)
      filter.listingType = alert.criteria.listingType;
    if (alert.criteria.propertyTypes?.length)
      filter.propertyType = { $in: alert.criteria.propertyTypes };
    if (alert.criteria.cities?.length)
      filter["location.city"] = {
        $in: alert.criteria.cities.map((c) => new RegExp(c, "i")),
      };
    ...
    if (alert.criteria.minBedrooms)
      filter["specifications.bedrooms"] = { $gte: alert.criteria.minBedrooms };
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching matching properties" });
  }
};
