...
exports.createReview = async (req, res) => {
  try {
    const { propertyId, rating, title, comment, ratings, pros, cons } =
      req.body;
...
exports.respondToReview = async (req, res) => {
  try {
    ...
    if (review.property.owner.toString() !== req.user.id)
      return res
        .status(403)
        .json({ success: false, message: "Only owner can respond" });
  ...
  try {
    const review = await Review.findByIdAndUpdate(
      req.params.id,
      {
        ...
        rejectionReason:
          req.body.status === "rejected" ? req.body.rejectionReason : undefined,
      },
      { returnDocument: "after" },
    );
    res.status(200).json({ success: true, review });
    ...
    res
      .status(500)
      .json({ success: false, message: "Error moderating review" });
  }
};
...
exports.getPendingReviews = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching pending reviews" });
  }
};
